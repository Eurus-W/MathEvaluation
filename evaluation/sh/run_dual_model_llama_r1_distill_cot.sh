...
run_single_setting() {
    ...
    if [[ "${RUN_ORDER}" == "benchmark_major" ]]; then
        for data_name in "${DATA_NAME_ITEMS[@]}"; do
            ...
            run_repeat_summary "${output_dir}" "${data_name}" "${temperature}" "${n_sampling}" "${model_path}" "${seeds_csv}"
        done
    else
        for seed in "${seed_items[@]}"; do
            if [[ ${#seed_items[@]} -gt 1 ]]; then
        ...
        run_repeat_summary "${output_dir}" "${DATA_NAMES}" "${temperature}" "${n_sampling}" "${model_path}" "${seeds_csv}"
    fi
}
